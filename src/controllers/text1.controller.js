...
import { Text1 } from "../models/text1.model.js";

export const createText1  = async(req,res) => {
try{
    const{ content,title} = req.body;
    console.log("data",content,title);
     
    const text = await Text1.create({
...
export const getText1 = async (req,res)=>{
    try{
    const text = await Text1.findAll();
...
export const getText1ById = async (req, res) => {
    try {
      const { textId } = req.params;
      console.log("All data", textId);
      const text = await Text1.findByPk(textId);
      if (!text) {
  ...
  export const updateText1 = async (req, res) => {
    try {
      const { textId } = req.params;
  ...
  export const deleteText1 = async (req, res) => {
    try {
      const { textId } = req.params;
      ...
      await Text1.destroy({ where: { id: textId } });
